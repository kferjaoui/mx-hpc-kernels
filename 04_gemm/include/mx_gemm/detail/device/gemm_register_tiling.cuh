                                                    ...
                                                    const size_t N_TILE, const size_t M_TILE, const size_t K_TILE) 
{
    ...
    for(int start_K_TILE_f4=0; start_K_TILE_f4 < K_f4; start_K_TILE_f4 += K_TILE_f4){
        ...
        for(size_t k=0; k < actual_K_TILE; ++k){
            auto a_base  = (threadIdx.y * Nr) * actual_K_TILE; // should i use `actual_K_TILE_f4 * jump` instead of `actual_K_TILE`? or same ?
            ...
            for(size_t ir=0; ir<Nr; ++ir)
                for(size_t jr=0; jr<Mr; ++jr){
                    C_subtile[jr + ir * Mr] += 
                            alpha * shmem[a_base + k + (ir * actual_K_TILE)] 
                                  * shmem[bt_base + k + (jr * K_TILE_PAD)]; 
                }
        }
...
// // Non-padded version of the vectorized kernel
// template <typename T, size_t Nr, size_t Mr>
// __global__ void gemm_register_tiled_vectorized_no_padding(const T alpha, const T* __restrict__ A, const T* __restrict__ BT, // BT is the transposed B matrix; we will load it in row-major order to make use of vectorized loads
//                                     const T beta, T* __restrict__ C,
//                                     const size_t N, const size_t M, const size_t K,
//                                     const size_t N_TILE, const size_t M_TILE, const size_t K_TILE) 
// {
...
//             if ( (local_tid_1d + idx_B4) < size_BT_tile_f4 ) shmem_f4[start_BT_tile_f4 + (local_tid_1d + idx_B4)] = 
//                                                                             BT_f4[offset_BT_tile_f4 + col_shift_f4 + K_f4 * row_shift_f4];
//         }
...
//                 for(size_t jr=0; jr<Mr; ++jr){
//                     C_subtile[jr + ir * Mr] += 
//                             alpha * shmem[local_start_row_i_of_A + k + (ir * actual_K_TILE)] 
//                                   * shmem[start_BT_tile_f4 * jump + local_start_row_j_of_BT + k + (jr * actual_K_TILE)]; 
//                 }
//         }
...
// TODO: READABILITY -> Best seperate the caller functions for the vectorized / non-vectorized versions
                    ...
                    const CUDA& cuda_policy)
{
    ...
    if constexpr (algo == CudaGemmAlgorithm::RegisterTiling) {
        const size_t shmemBytes = sizeof(T) * ( K_TILE * (N_TILE + M_TILE) ); // Needs to hold one A_tile and one B_tile
    ...
    //     const size_t shmemBytes = sizeof(T) * ( K_TILE * (N_TILE + M_TILE) ); // Needs to hold one A_tile and one B_tile
    //     gemm_register_tiled_vectorized_no_padding<T, N_SUBTILE, M_SUBTILE><<<grid, threadBlock, shmemBytes, stream>>>(
    //                             alpha, dA, dB, beta, dC, N, M, K, N_TILE, M_TILE, K_TILE);
    // }
    else if constexpr (algo == CudaGemmAlgorithm::RegisterTilingVectorized) {
        constexpr auto K_TILE_PAD = K_TILE + 1; // Padding for the BT chunk in shmem
        const size_t shmemBytes = sizeof(T) * ( N_TILE * K_TILE + M_TILE * K_TILE_PAD ); // Hold one A_tile and one padded B_tile
